...
export default function MoviePage() {
  ...
  if (genre.loading) return <LoadingPage page="phim-le"/>
  if (genre.error) {
    navigate("/error")
  }
  return ( 
      ...
      <div className="py-32 px-20">
        ...
        <div> 
        {
            genre.data.genres.map((genre,key)=>{
              return(
                <ListFilm key={key} title={genre.name} genreId={genre.id} films={genre.films.filter(film=> film.filmType.name === "Movie")} openModal={openModal}/>            
              )
            })
          }
        </div>
